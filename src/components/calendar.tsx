...
import { db, auth } from "@/pages/api/firebase/firebase"; // Adjust to your actual Firebase Firestore import
import EventList from "@/components/EventList"; // Adjust to your actual EventList component import
...
const Calendar: React.FC = () => {
  ...
  const [loading, setLoading] = useState(true);
  const [currentDate, setCurrentDate] = useState(new Date());
  const [attendedEvents, setAttendedEvents] = useState<string[]>([]); // Store attended event IDs
  ...
  for (let i = 1; i <= daysInMonth; i++) {
    ...
    let dayClass = "";
    if (hasEvent) {
      dayClass = "bg-green-300"; // Blue for events user is not attending
    }

    days.push(
      <div
        key={i}
        className={`calendar-day ${isSelected ? "selected" : ""} ${dayClass}`}
        onClick={() => handleDateClick(i)}
      >
        {i}
      </div>
